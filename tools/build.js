...
function lessonPage(item, index) {
  ...
  const content = guide ? installationGuide(guide, item.slug) : productGuide ? deepProductGuide(productGuide, item.slug) : standardLesson(item);
  const lessonType = guide ? '<span class="pill">Windows 10/11</span>' : productGuide ? translated('lesson.deep', 'Deep lesson', 'שיעור מעמיק', 'span', ' class="pill"') : '<span class="pill">10–15 min</span>';
